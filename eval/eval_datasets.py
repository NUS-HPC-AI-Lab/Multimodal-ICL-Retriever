...
class CaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.dataset_name == "textcaps":
            image = Image.open(
                os.path.join(
                    self.image_train_dir_path, self.annotations[idx]["image_name"] + ".jpg"
                )
                if self.annotations[idx]["set_name"] == "train"
                else os.path.join(
                    self.image_val_dir_path, self.annotations[idx]["image_name"] + ".jpg"
                )
            )
        image.load()
