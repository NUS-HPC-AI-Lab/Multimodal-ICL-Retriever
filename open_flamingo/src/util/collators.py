...
class DataCollatorWithPaddingAndCuda:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> BatchEncoding:
        ...
        has_text = features[0]["input_ids"] is not None
        ...
        if has_text:
            ...
            for i in range(len(features)):
                image_lists.append(features[i]['image_inputs'])
            batch['image_inputs'] = torch.tensor(image_lists)
...
class IBNDataCollatorWithPadding:
    ...
    def __call__(self, samples: List[InputSample]) -> BatchEncoding:
        ...
        questions_image_tensor, questions_text_tensor = [], []
        ctxs_image_tensor, ctxs_text_tensor, ctxs_attention_mask = [], [], []
        ctx_indices = []
        eid2idx = {}
            ...
            for i, ctxs in enumerate(all_ctxs):
                ...
                for eid in ctxs:
                    if eid in eid2idx:
                        ...
                        ctxs_image_tensor.append(self.encoded_index[eid]['image_inputs'])
                        ctxs_text_tensor.append(self.encoded_index[eid]['input_ids'])
                        ctxs_attention_mask.append(self.encoded_index[eid]['attention_mask'])
                ctx_indices.append(indices)

            questions_image_tensor.append(sample.question_image_ids)
        ...
        batch = {"questions_image_tensor": torch.tensor(questions_image_tensor).squeeze(1),
                 "ctxs_image_tensor": torch.tensor(ctxs_image_tensor).squeeze(1),
                 "ctxs_text_tensor": torch.tensor(ctxs_text_tensor),
                 "ctxs_attn_mask": torch.tensor(ctxs_attention_mask),
